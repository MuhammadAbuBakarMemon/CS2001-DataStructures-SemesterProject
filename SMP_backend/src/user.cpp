...
#include <iostream>
#include <string>
#include "P:/Github/Repos/CS2001-DataStructures-SemesterProject/SMP_backend/include/core/user.hpp"
...
User* UserManagement::search(const string& username) const {
    int index = hashFunction(username);       // get hash index
    User* head = hashTable[index];            // get linked list head at that index
    return searchUserInList(head, username);  // use helper function to search the linked list
}